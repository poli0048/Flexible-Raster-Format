...
//Destructor
FRFImage::~FRFImage() { Clear(); /*std::cerr << "FRF object destroyed.\r\n";*/ }
...
//Try to load an FRF image from a buffer. On failure, create an empty image
bool FRFImage::LoadFromRAM(std::vector<uint8_t> const & Buffer) {
	Clear();
	if (Buffer.size() < 16U)
		return false;
	auto iter = Buffer.cbegin();
	...
	imageHeight  = decodeField_uint16(iter);
	if (MagicNumber != (uint64_t) 3197395143525533696U) {
		fprintf(stderr,"Error in FRFImage::LoadFromRAM. Magic number is not correct - Corrupt file? Aborting load.\r\n");
		Clear();
		return false;
	}
	...
	//Now read information blocks until we have decoded the entire header (we will know because of the End-Of-Header block)
	while (true) {
		if (Buffer.end() - iter < 6U) {
			fprintf(stderr,"Error in FRFImage::LoadFromRAM. Not enough bytes in file. Aborting load.\r\n");
			Clear();
			return false;
		}
		uint16_t BlockCode = decodeField_uint16(iter);
		uint32_t Size      = decodeField_uint32(iter);
		if (Size < 6U) {
			fprintf(stderr,"Error in FRFImage::LoadFromRAM. Size field in header info block has value less than 6. Aborting load.\r\n");
			Clear();
			return false;
		}
		uint32_t PayloadSize = Size - (uint32_t) 6U;
		std::vector<uint8_t> PayloadBuffer;
		if (PayloadSize > 0U) {
			PayloadBuffer.reserve(PayloadSize);
			PayloadBuffer.insert(PayloadBuffer.end(), iter, iter + PayloadSize);
			iter += PayloadSize;
		}
		
		if (BlockCode == 0U) {
			//Decode Layer Manifest Block
			if (PayloadBuffer.size() < 32U) {
				fprintf(stderr,"Error in FRFImage::LoadFromRAM. Too few bytes in layer manifest payload. Aborting load.\r\n");
				Clear();
				return false;
			}
			auto payloadIter = PayloadBuffer.cbegin();
			alphaLayerIndex = decodeField_uint16(payloadIter);
			unsigned int bytesLeft = ((unsigned int) PayloadBuffer.size()) - 2U;
			
			while (bytesLeft > 0U) {
				FRFLayer * newLayer = this->AddLayer();
				deserialize_LayerManifestSection(newLayer, payloadIter, bytesLeft);
			}
			
		}
		else if (BlockCode == 1U) {
			//Decode Visualizations Block
			if (PayloadBuffer.size() < 16U) {
				fprintf(stderr,"Error in FRFImage::LoadFromRAM. Too few bytes in visualizations block payload. Aborting load.\r\n");
				Clear();
				return false;
			}
			auto payloadIter = PayloadBuffer.cbegin();
			unsigned int bytesLeft = (unsigned int) PayloadBuffer.size();
			while (bytesLeft > 0U) {
				FRFVisualization * vis = deserialize_VisualizationSection(payloadIter, bytesLeft);
				VisualizationManifest.push_back(vis);
			}
		}
		else if (BlockCode == 2U) {
			//Decode Geo-Tagging Block
			if (PayloadBuffer.size() != 108U)
				fprintf(stderr,"Error in FRFImage::LoadFromRAM. Incorrect payload size for Geo-Tagging block. Dropping block.\r\n");
			else {
				auto payloadIter = PayloadBuffer.cbegin();
				GeoTagData = deserialize_GeoTagPayload(payloadIter);
			}
		}
		else if (BlockCode == 3U) {
			//Decode Geo-Registration Block
			auto payloadIter = PayloadBuffer.cbegin();
			unsigned int payloadSize = (unsigned int) PayloadBuffer.size();
			GeoRegistrationData = deserialize_GeoRegistrationPayload(payloadIter, payloadSize);
		}
		else if (BlockCode == 4U) {
			//Decode Camera Information Block
			auto payloadIter = PayloadBuffer.cbegin();
			...
			while (bytesLeft > 0U) {
				if (bytesLeft < 4U) {
					fprintf(stderr,"Warning in FRFImage::LoadFromRAM. Dropping partial Cam Info Tag.\r\n");
					break;
				}
				
				uint16_t TagCode = decodeField_uint16(payloadIter);
				uint16_t TagPayloadSize = decodeField_uint16(payloadIter);
				bytesLeft -= 4U;
				if (TagPayloadSize > bytesLeft) {
					fprintf(stderr,"Warning in FRFImage::LoadFromRAM. Dropping partial Cam Info Tag.\r\n");
					break;
				...
				std::vector<uint8_t> & TagData = CameraInformationTags[TagCode];
				TagData.clear();
				TagData.insert(TagData.begin(), payloadIter, payloadIter + TagPayloadSize);
				bytesLeft -= TagPayloadSize;
			}
		}
		else if (BlockCode == 5U) {
			//Decode Custom Block
			if (PayloadBuffer.size() < 8U)
				fprintf(stderr,"Error in FRFImage::LoadFromRAM. Custom block payload too small. Dropping block.\r\n");
			else {
				auto payloadIter = PayloadBuffer.cbegin();
				uint64_t CustomBlockCode = decodeField_uint64(payloadIter);
				FRFCustomBlock & block = customBlocks[CustomBlockCode];
				block.CustomBlockCode = CustomBlockCode;
				block.BlockPayload.clear();
				block.BlockPayload.insert(block.BlockPayload.begin(), payloadIter, PayloadBuffer.cend());
			}
		}
		else if (BlockCode == 6U) {
			//Process End-Of-Header block
	...
	for (FRFLayer * layer : Layers) {
		layer->AllocateStorage();
		uint64_t totalLayerSize = layer->GetTotalBytesWithValidityMask();
		if ((size_t) (Buffer.end() - iter) < size_t(totalLayerSize)) {
			fprintf(stderr,"Error in FRFImage::LoadFromRAM. Not enough bytes in file. Aborting load.\r\n");
			Clear();
			return false;
		}
		std::copy(iter, iter + totalLayerSize, layer->Data.begin());
		iter += totalLayerSize;
	}
	
	if (iter != Buffer.end())
		fprintf(stderr,"Warning in FRFImage::LoadFromRAM. There are extra bytes left in the file (after load). File may be corrupt.\r\n");
	return true;
}
